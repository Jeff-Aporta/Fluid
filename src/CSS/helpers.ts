...
/* Ayudante de unidades */
const u = (val: TStrNum) => new UnitValue(val);

/* Crea operacion */
const createOp = (op: string) => (...args: (Expr | TStrNum)[]) => new Operation(op, args);

/* Clamp para CSS */
const clampCss = (min: number, max: number, unit: string) => `clamp(${min}${unit}, ${min}${unit} + (${max} - ${min}) * ((100vw - 400px) / (1100 - 400)), ${max}${unit})`;

/* Infiere unidad */
const inferUnit = (val: TStrNum, unit: string = "%") => typeof val === "number" ? `${val}${unit}` : val.endsWith(unit) ? val : `${val}${unit}`;

/* Agrega sufijo si es numero */
const addSufix = (v: TStrNum, sufix: string) => {
    if (typeof v === 'number') return `${v}${sufix}`;
    if (typeof v === 'string') {
        if (v.endsWith(sufix)) return v;
        if (sufix.trim() === "!important") return `${v}${sufix}`;
        if (/^-?\d+(\.\d+)?$/.test(v)) return `${v}${sufix}`;
    }
    return v;
};

/* Ayudante generico para espaciado */
const spacingHelper = (args: any[]) => {
    let unitFn = px;
    let values = args;
...
/* Genera gradiente lineal */ export const linearGradient = (dir: string | number | { dir?: string | number, to?: string, stops: (string | [string, number])[] }, stops?: (string | [string, number])[]) => {
    ...
    if (typeof dir === 'object' && !Array.isArray(dir)) {
        ...
        direction = typeof dir === 'number' ? `${dir}deg` : dir as string;
        colorStops = stops || [];
    }
...
/* Genera gradiente radial */ export const radialGradient = (shape: string | { shape?: string, at?: string | { x: number, y: number }, stops: (string | [string, number])[] }, stops?: (string | [string, number])[]) => {
    ...
    if (typeof shape === 'object' && !Array.isArray(shape)) {
        const { shape: s = "circle", at, stops: st } = shape;
        let atStr = "";
        if (at) {
            if (typeof at === 'object') atStr = ` at ${typeof at.x === 'number' ? `${at.x}px` : at.x} ${typeof at.y === 'number' ? `${at.y}px` : at.y}`;
            else atStr = ` at ${at}`;
        }
        shapeStr = `${s}${atStr}`;
        colorStops = st;
    } else {
        shapeStr = shape as string;
        colorStops = stops || [];
    }
    const formattedStops = colorStops.map(s => Array.isArray(s) ? `${s[0]} ${s[1]}%` : s).join(', ');
...
/* Genera transformacion */ export const transform = (initial?: Record<string, TStrNum>) => {
    ...
    const builder = {
        translate: (x: TStrNum, y: TStrNum) => (ops.push(`translate(${typeof x === 'number' ? `${x ?? 0}px` : x}, ${typeof y === 'number' ? `${y ?? 0}px` : y})`), builder),
        translateX: (v: TStrNum) => (add("translateX", v, 0, "px"), builder),
        translateY: (v: TStrNum) => (add("translateY", v, 0, "px"), builder),
        translateZ: (v: TStrNum) => (add("translateZ", v, 0, "px"), builder),
        scale: (v: TStrNum) => (add("scale", v, 1), builder),
        scaleX: (v: TStrNum) => (add("scaleX", v, 1), builder),
        scaleY: (v: TStrNum) => (add("scaleY", v, 1), builder),
        rotate: (v: TStrNum) => (add("rotate", v, 0, "deg"), builder),
        rotateX: (v: TStrNum) => (add("rotateX", v, 0, "deg"), builder),
        rotateY: (v: TStrNum) => (add("rotateY", v, 0, "deg"), builder),
        skew: (x: TStrNum, y: TStrNum) => (ops.push(`skew(${typeof x === 'number' ? `${x ?? 0}deg` : x}, ${typeof y === 'number' ? `${y ?? 0}deg` : y})`), builder),
        skewX: (v: TStrNum) => (add("skewX", v, 0, "deg"), builder),
        skewY: (v: TStrNum) => (add("skewY", v, 0, "deg"), builder),
...
/* Ayudante de union */ export const join = (...args: any[]) => {
    ...
    if (args.length > 0 && typeof args[args.length - 1] === 'object' && args[args.length - 1] !== null && !Array.isArray(args[args.length - 1])) {
        config = { ...config, ...args[args.length - 1] };
        values = args.slice(0, -1);
    }
    return values.map(v => {
        if (typeof v === 'number') { if (v === 0) return '0'; return `${v}${config.unit}`; }
        return v;
...
/* Genera sombra paralela */ export const dropShadow = ({ dX = 0, dY = 0, blur = 0, color = "black" }: { dX?: TStrNum; dY?: TStrNum; blur?: TStrNum; color?: string }) => {
    const px = (v: TStrNum) => typeof v === 'number' ? `${v}px` : v;
    return `drop-shadow(${px(dX)} ${px(dY)} ${px(blur)} ${color})`;
};

/* Otros filtros */
const filterUnit = (v: TStrNum, unit: string = "") => typeof v === 'number' ? `${v}${unit}` : v;
export const blur = (v: TStrNum) => `blur(${filterUnit(v, "px")})`;
export const brightness = (v: TStrNum) => `brightness(${v})`;
export const contrast = (v: TStrNum) => `contrast(${v})`;
export const grayscale = (v: TStrNum) => `grayscale(${v})`;
export const hueRotate = (v: TStrNum) => `hue-rotate(${filterUnit(v, "deg")})`;
export const invert = (v: TStrNum) => `invert(${v})`;
export const opacity = (v: TStrNum) => `opacity(${v})`;
export const saturate = (v: TStrNum) => `saturate(${v})`;
