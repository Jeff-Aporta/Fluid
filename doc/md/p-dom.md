...
flowchart LR
    ...
    EmmetParser --> DomCreate["document.createElement()"]
    
    InputAnalysis -- "HTMLElement" --> ExistingNode[Referencia al Nodo]
    
    InputAnalysis -- "Función" --> ExecFn[Ejecutar Función con Helpers]
    ExecFn --> RecursiveCall["Recursión a element()"]
    
    InputAnalysis -- "JSON" --> JsonParser[Iterar Claves y Props]
    JsonParser --> RecursiveCreate[Crear Hijos Recursivamente]
    
    DomCreate --> BuilderInit[Inicializar Builder Proxy]
    ExistingNode --> BuilderInit
    RecursiveCall --> BuilderInit
    RecursiveCreate --> BuilderInit
    
    BuilderInit --> Return(["Retornar HTMLElementBuilder"])
```
...
| Método | Firma Técnica | Detalle de Implementación |
| :--- | :--- | :--- |
| `id` | `(v: string) => this` | Asignación directa a `el.id`. Sobrescribe cualquier Identificador previo. |
| `addClass` | `(cls: string \| string[]) => this` |  |
| `style` | `(props: Partial<CSSStyleDeclaration>) => this` | Itera sobre las propiedades del objeto y las asigna al objeto `el.style`. **Nota**: No emplea el motor de inferencia CSS completo; su uso se restringe a estilos en línea directos. |
| `assign` | `(props: Record<string, any>) => this` | Utiliza `Object.assign`. Crucial para propiedades que carecen de atributos HTML correspondientes (v.gr., `value` en inputs, `srcObject` en video). |
...
```mermaid
flowchart TD
    Start(["Entrada: Propiedad JS"]) --> NormalizeKey["Normalizar Clave: camelCase -> kebab-case"]
    NormalizeKey --> CheckValueType{"¿Tipo de Valor?"}
    
    CheckValueType -- "Función" --> Execute["Ejecutar fn(HELPERS)"]
    Execute --> CheckValueType
    
    CheckValueType -- "Objeto" --> IsArray{"¿Es Array?"}
    IsArray -- "Sí" --> JoinArray["Unir valores (alternativa)"]
    IsArray -- "No" --> Recursion["Recursión: Nueva Regla CSS"]
    
    CheckValueType -- "Número" --> IsUnitless{"¿En Lista Exclusión?"}
    IsUnitless -- "Sí (z-index, opacity)" --> ToString["Convertir a Cadena"]
    IsUnitless -- "No (width, padding)" --> AddPx["Concatenar 'px'"]
    
    CheckValueType -- "Cadena" --> Trim["Limpiar Espacios"]
    
    JoinArray --> AddToBuffer["Añadir al Búfer CSS"]
    Recursion --> AddToBuffer
    ToString --> AddToBuffer
    AddPx --> AddToBuffer
    Trim --> AddToBuffer
    
    AddToBuffer --> End(["Siguiente Propiedad"])
```
