            ...
            hook.doneEach(function () {
                ...
                const setupZoom = (container, svg) => {
                    ...
                    if (window.mediumZoom) {
                        ...
                        let zoom = window.mediumZoom(img, { background: '#0f172a' });

                        img.addEventListener('click', (e) => {
                            e.stopPropagation();

                            // 1. Detach old zoom to clear state
                            zoom.detach();

                            // 2. Refresh Image Source from CURRENT SVG
                            ...
                            // 3. Re-attach zoom with fresh image
                            zoom = window.mediumZoom(img, { background: '#0f172a' });
                        ...
                        // Re-attach listener for the NEW zoom instance's close event is tricky 
                        // because 'zoom' variable changes. 
                        // Instead, we can use a global listener or just handle opacity on the img directly
                        // But mediumZoom doesn't fire events on the img element itself usually.
                        // Let's attach the 'closed' listener to the NEW zoom instance inside the click handler?
                        // No, zoom.open() is async-ish. 

                        // Better approach: Use a consistent zoom instance but update the 'src' 
                        // AND tell mediumZoom to re-scan or update? mediumZoom doesn't have an 'update' method for src change easily.
                        // Detach/Attach is the most robust way to force it to see the new src.

                        // We need to ensure the 'closed' event is handled for the NEW instance.
                        // We can wrap the zoom creation in a helper.
                    }
                };

                // ... (rest of the logic remains similar, but we need to implement the helper above correctly)
                // Let's refine the replacement content to be self-contained and correct.
                ...
                hook.doneEach(function () {
                    ...
                    const setupZoom = (container, svg) => {
                        ...
                        if (window.mediumZoom) {
                            const img = document.createElement('img');
                            img.classList.add('zoom-overlay'); // Mark for detection
                            ...
                            // Function to update image source from current SVG state
                            ...
                            // Update on mousemove to ensure freshness (User Request)
                            img.addEventListener('mousemove', updateImageSource);

                            img.addEventListener('click', (e) => {
                                e.stopPropagation();
                                updateImageSource(); // Force update before opening
                                img.style.opacity = '1';
                                zoom.open();
                            });
